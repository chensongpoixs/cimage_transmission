...
namespace chen {
	cwebsocket_wan_server  g_websocket_wan_server;
	cwebsocket_wan_server::~cwebsocket_wan_server()
	{
	}
	bool cwebsocket_wan_server::init()
	{
		...
		m_websocket_server_ptr->set_connect_callback(
			std::bind(&cwebsocket_wan_server::on_connect, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3)
		);

		m_websocket_server_ptr->set_msg_callback(
			std::bind(&cwebsocket_wan_server::on_msg_receive, this, std::placeholders::_1, std::placeholders::_2
				, std::placeholders::_3)
		);

		m_websocket_server_ptr->set_disconnect_callback(
			std::bind(&cwebsocket_wan_server::on_disconnect, this, std::placeholders::_1)
		);

		return true;
	}
	void cwebsocket_wan_server::destroy()
	{
		if (m_websocket_server_ptr)
		{
	...
	bool cwebsocket_wan_server::startup()
	{
		if (!m_websocket_server_ptr)
		{
			return false;
	...
	void cwebsocket_wan_server::update(uint32 uDeltaTime)
	{
		...
		if (m_update_timer >= UPDATE_TIMER)
		{
			...
			//for (std::map<uint64, cwebsocket_wan_session*>::iterator iter = m_session_map.begin(); iter != m_session_map.end(); ++iter)
			//{
	...
	void cwebsocket_wan_server::shutdown()
	{
		if (m_websocket_server_ptr)
		{
	...
	void cwebsocket_wan_server::on_connect(uint64_t session_id, const char* ip, uint16_t port)
	{
		NORMAL_EX_LOG("[session_id = %u][ip = %s][port = %u]", session_id, ip, port);
		std::lock_guard< clock_type> lock(m_session_lock);
		...
		//std::map<uint64, cwebsocket_wan_session*>::iterator iter = m_session_map.find(session_id);
		...
		//	return;
		//}
		//cwebsocket_wan_session* session_ptr = new cwebsocket_wan_session();
		//if (!session_ptr)
		//{
		//	WARNING_EX_LOG("alloc websocket wan session failed !!!");
	...
	void cwebsocket_wan_server::on_msg_receive(uint64_t session_id, const void* p, uint32 size)
	{
		/*std::map<uint64, cwebsocket_wan_session*>::iterator iter = m_session_map.find(session_id);
		if (iter != m_session_map.end())
		{
	...
	void cwebsocket_wan_server::on_disconnect(uint64_t session_id)
	{
	...
	void cwebsocket_wan_server::broadcast_msg(uint16 msg_id, const void* p, uint32 size)
	{
	
		std::lock_guard< clock_type> lock(m_session_lock);
		for (const uint32& session_id : m_sessions)
		{
			send_msg(session_id, msg_id, p, size);
		}
	}
	 
	void cwebsocket_wan_server::close(uint32 session_id)
	{
		if (m_websocket_server_ptr)
		{
			m_websocket_server_ptr->close(session_id);
